import { ChainId, Token, Pair, TokenAmount, WBTT, Price } from '../src'
...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(DAS, '100'), new TokenAmount(WBTT[ChainId.FUJI], '100'), ChainId.FUJI)).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      ...
      expect(Pair.getAddress(DAS, CON, ChainId.BITTORRENT_MAINNET)).toEqual('0xaf5fdF7De60779DA4409498DfdfA3803984e8536')
    })
  })
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WBTT[ChainId.FUJI])).toThrow('TOKEN')
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(CON, '101'), new TokenAmount(DAS, '100'), ChainId.FUJI).reserveOf(WBTT[ChainId.FUJI])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(DAS, '100'), new TokenAmount(CON, '100'), ChainId.FUJI).involvesToken(WBTT[ChainId.FUJI])
    ).toEqual(false)
  })
})
